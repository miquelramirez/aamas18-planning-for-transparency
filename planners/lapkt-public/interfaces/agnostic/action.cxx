...
void Action::redefine( Fluent_Vec precs, Fluent_Vec adds, Fluent_Vec dels )
{
	// define the precondition, adds and deletes
	redefine_fluent_list( precs, prec_vec(), prec_set() );
	redefine_fluent_list( adds, add_vec(), add_set() );
	redefine_fluent_list( dels, del_vec(), del_set() );
	redefine_fluent_list( dels, edel_vec(), edel_set() );
...
void Action::redefine_fluent_list(  Fluent_Vec in, Fluent_Vec& fluent_list, Fluent_Set& fluent_set )
{
	fluent_list.clear();
	fluent_set.reset();
	for ( unsigned k = 0; k < in.size(); k++ )
