...
// Belief State
inline void Hash_Key::add( std::vector<double>& k )
{

	if ( k.empty() )
	{
		unsigned k2 = 0;
		m_code = jenkins_hash( (ub1*)&k2, sizeof(double), m_code );
		return;
	}

  //unsigned zero = 0;
  //unsigned one = 1;
  //std::cout << "M_code: " << jenkins_hash( (ub1*)(&k[0]), sizeof(double), m_code ) << std::endl;
  //std::cout << "M_code1: " << jenkins_hash( (ub1*)(&one), sizeof(double), m_code ) << std::endl;
  //std::cout << "M_code0: " << jenkins_hash( (ub1*)(&zero), sizeof(double), m_code )<< std::endl;

	std::sort( k.begin(), k.end() );

	m_code = jenkins_hash( (ub1*)(&k[0]), sizeof(double), m_code );
	for ( unsigned i = 1; i < k.size(); i++ )
	{
		m_code = jenkins_hash( (ub1*)(&k[i]), sizeof(double), m_code );
	}
