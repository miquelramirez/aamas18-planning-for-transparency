...
#ifndef __RECURSIVE_NOVELTY_PARTITION__
#define __RECURSIVE_NOVELTY_PARTITION__
...
class Recursive_Novelty_Partition : public Heuristic<State>{
public:


	Recursive_Novelty_Partition( const Search_Model& prob, unsigned max_arity = 1, const unsigned max_MB = 2048 ) 
	    : Heuristic<State>( prob ), m_strips_model( prob.task() ), m_max_memory_size_MB(max_MB), m_always_full_state(true), m_verbose( true ), m_partition_size(0) {
		
		set_arity(max_arity, 1);
		set_max_recursion(1);
	}

	virtual ~Recursive_Novelty_Partition() {
	}


	void init() {
		typedef typename std::vector< std::vector< std::vector<Search_Node*> > >::iterator        Node_2Vec_Ptr_It;
		...
		for(Node_2Vec_Ptr_It it_p2 = m_nodes_tuples_by_partition.begin();it_p2 != m_nodes_tuples_by_partition.end(); it_p2++)
			for(Node_Vec_Ptr_It it_p = it_p2->begin(); it_p != it_p2->end(); it_p++)		
	...
	void set_max_recursion( unsigned max_rec ) {m_max_recursion = max_rec; }
...
protected:
	...
	void check_table_size( Search_Node* n ){		
		...
		if(  m_nodes_tuples_by_partition[ n->partition() ].empty() )
			m_nodes_tuples_by_partition[ n->partition() ].resize( m_num_tuples );	       
	}
	...
	void compute(  Search_Node* n, unsigned& novelty ) 
	{
		...
		for(unsigned i = 0; i < m_max_recursion; i++){
#ifdef DEBUG
			if ( m_verbose )
			...
			if(n->parent() == nullptr || m_always_full_state)
				...
				new_covers = (n->partition() == n->parent()->partition() and (i <= n->parent()->novelty())) ?  cover_tuples_op( n, i ) : cover_tuples( n, i );
			//new_covers =  cover_tuples( n, i );
#ifdef DEBUG
		...
		novelty = m_max_recursion+1;
		
	}
	       
	bool cover_tuples( Search_Node* n, unsigned recursion  )
	{
		...
		for( unsigned idx = 0; idx < n_combinations; idx++ ){
			 ...
			 */
			idx2tuple( tuple, fl, idx, m_arity );

			/**
			 * Check if tuple is covered
			 */
			unsigned tuple_idx = tuple2idx( tuple, m_arity );
			...
			unsigned tsize = m_nodes_tuples_by_partition[ n->partition() ][ tuple_idx ].size();
			bool cover_new_tuple = ( tsize <= recursion ) ? true : ( is_better( m_nodes_tuples_by_partition[ n->partition() ][tuple_idx][recursion], n  ) ? true : false);
			
			if( cover_new_tuple ){
				if(tsize <= recursion)
					m_nodes_tuples_by_partition[ n->partition() ][ tuple_idx ].push_back( (Search_Node*) n);
				else
					m_nodes_tuples_by_partition[ n->partition() ][ tuple_idx ][recursion] = (Search_Node*) n;
	...
	bool    cover_tuples_op( Search_Node* n, unsigned recursion  )
	{
		...
		unsigned atoms_arity = m_arity - 1;
	...
	std::vector< std::vector< std::vector<Search_Node*> > >    m_nodes_tuples_by_partition;
        unsigned                m_arity;
