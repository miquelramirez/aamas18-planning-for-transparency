...
class RP_H1_Heuristic : public Heuristic<State> {
public:

	typedef STRIPS_Problem::Best_Supporter 	Best_Supporter;

	RP_H1_Heuristic( const Search_Model& prob , const std::vector<unsigned>& observations)
	: Heuristic<State>( prob ), m_strips_model( prob.task() ), eval_func(m_values) , m_observations(observations){
...
protected:
  ...
  void compute() {
	...
	void	compute(std::vector<unsigned> obs_seq_so_far  )
	{
        ...
        //TODO: Add penalty for having not seen observations before
        // Introducing penalty function here relying on whether an observation is seen before or not
        auto address = std::find(m_observations.begin(), m_observations.end(), a.index());
        int index = std::distance(m_observations.begin(), address);

        float h_observation_sequence_left = 0;

        for(std::vector<unsigned>::iterator observations_it = address; observations_it != m_observations.end(); ++observations_it){

          const Action& o = *(m_strips_model.actions()[*observations_it]);
          float h_temp = eval_func(o.prec_vec().begin(), o.prec_vec().end());

          h_observation_sequence_left = (h_observation_sequence_left > h_temp ? h_temp : h_observation_sequence_left);
        }

				float h_pre = eval_func( a.prec_vec().begin(), a.prec_vec().end() ) + h_observation_sequence_left + 1;
...
protected:
  ...
  std::vector<unsigned> m_observations;
};
