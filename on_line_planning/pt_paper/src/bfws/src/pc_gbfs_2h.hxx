...
#ifndef __PER_CALL_SINGLE_QUEUE_DOUBLE_HEURISTIC_GREEDY_BEST_FIRST_SEARCH__
#define __PER_CALL_SINGLE_QUEUE_DOUBLE_HEURISTIC_GREEDY_BEST_FIRST_SEARCH__
...
public:
	...
	Node( State* s, float cost, Action_Idx action, Node<Search_Model,State>* parent )
	    ...
	    m_beta(1.0f), m_total_delta(0), m_best_negated_obs_cost(0.0f)
  {
		m_g = ( parent ? parent->m_g + cost : 0.0f);
		m_g_unit = ( parent ? parent->m_g_unit + 1.0f : 0.0f);
		m_total_delta = ( parent ? parent->m_total_delta : 0.0f);
		m_post = ( parent ? parent->m_post : s->m_problem.m_priors);

    if(!parent)
    {
      for(auto i : m_post){
        m_best_negated_obs_cost.push_back(infty);

        std::vector<unsigned int> tempt_ui;
        std::vector<int> tempt_i;

        m_best_negated_obs_plan.push_back(tempt_ui);
        m_best_negated_obs_rel_plan.push_back(tempt_i);
      }
    }
    else
    {
      m_best_negated_obs_cost = parent->m_best_negated_obs_cost;
      m_best_negated_obs_plan = parent->m_best_negated_obs_plan;
      m_best_negated_obs_rel_plan = parent->m_best_negated_obs_rel_plan;
    }
	...
	// Added For PfT
	float m_total_delta;
	void set_total_delta(float delta){m_total_delta = delta;}
  std::vector<double> m_post;
  unsigned int m_goal_index;

  std::vector<float> m_best_negated_obs_cost;
  std::vector<std::vector<int>> m_best_negated_obs_rel_plan;
  std::vector<std::vector<unsigned int>> m_best_negated_obs_plan;

  std::vector<double>& og() {return m_priors;}
  void set_og(std::vector<double> priors) {m_priors = priors;}
	// _____________
...
public:
	...
	: m_problem( search_problem ), m_exp_count(0), m_gen_count(0), m_pruned_B_count(0),
	  m_dead_end_count(0), m_open_repl_count(0),m_B( infty ), m_time_budget(infty), m_max_h2n(no_such_index), m_max_h1n(0), m_verbose( true ), m_ignore_novelty(false),m_counter(0) {
	...
	void	start( float B = infty) {
		...
		eval(m_root);
    m_root->state()->set_bs(m_root->m_post);
    m_root->state()->update_hash();
		...
		inc_gen();
         ...
         void	set_arity( float v, unsigned p = 0 ){ m_first_h->set_arity( v, p); }
        void	set_max_recursion( unsigned p  ){ m_first_h->set_max_recursion(p); }
	...
	void			eval( Search_Node* candidate ) {
		...
		float float_hn;
		m_second_h->eval( candidate, float_hn );
		candidate->h2n() = (int) float_hn;
		...
		if ( candidate->goals_unachieved() == 0 || m_ignore_novelty ){
			...
			candidate->novelty() = candidate->h1n();
		}
		...
		if(candidate->h1n() > m_max_h1n ){
			m_max_h1n = candidate->h1n();
			if ( m_verbose ) {
				//std::cout << "novel--[" << m_max_h1n <<"]--" << std::endl;
				//std::cout << "[ n:" << candidate->h1n()  <<" - hl:" << candidate->h2n() <<" - #g:" << candidate->goals_unachieved() <<" - h_a:" << candidate->h3n() <<" - gn: " << candidate->gn()  <<"]" << std::endl;
			}
		}
