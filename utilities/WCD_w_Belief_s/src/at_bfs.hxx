...
public:
	...
	Node(State* s, float cost, Action_Idx action, Node<State>* parent, std::vector<aptk::Fluent_Vec>& goal_set)
	: m_state( s ), m_parent( parent ), m_action(action), m_g( 0 ), human_goal_set_belief(goal_set) {
...
public:
	...
	float		m_g;
	float		m_f;
	std::vector<aptk::Fluent_Vec> human_goal_set_belief;
};
...
public:
	...
	AT_BFS_SQ_SH( 	const Search_Model& search_problem, std::vector<aptk::Fluent_Vec>& goal_set)
	...
	m_exp_count(0), m_gen_count(0), m_pruned_B_count(0), m_dead_end_count(0), m_open_repl_count(0),
	  m_B( infty ), m_time_budget(infty), m_greedy(false), m_delay_eval(true), init_goal_set(goal_set) {
	...
	void	start() {
		m_B = infty;
		m_root = new Search_Node( m_problem.init(), 0.0f, no_op, NULL, init_goal_set );
		#ifdef DEBUG
	...
	virtual void 			process(  Search_Node *head ) {
		...
		for (unsigned i = 0; i < app_set.size(); ++i ) {
			...
			// TODO Code goes here to change human_goal_set_belief
			Search_Node* n = new Search_Node( succ, m_problem.cost( *(head->state()), a ), a, head, head->human_goal_set_belief );
...
protected:
	...
	bool                                    m_greedy;
	bool                                    m_delay_eval;
	std::vector<aptk::Fluent_Vec> init_goal_set;
};
