...
namespace aptk
{
	...
	public:
		...
		void set_markov(bool markov){m_markov = markov;}
		void set_true_init(Fluent_Vec s){m_true_init = s;}
		void set_priors(const std::vector<double> priors){m_priors = priors;}
		void set_precision(unsigned p){m_precision = p;}
		void set_beta(float beta) {m_beta = beta;}
		void set_df(float df) {m_discount_factor = df;}
		void set_nan(bool n) {m_negated_as_neutral = n;}
		void set_v_beta(bool v) {m_variable_beta = v;}
		void set_pr_verbose(bool v) {m_pr_verbose = v;}
		void set_h_verbose(bool v) {m_h_verbose = v;}
		void set_optimal(bool v) {m_optimal = v;}
		void set_observations(std::vector<unsigned int> o){m_top_level_observations = o;}
    void set_theta(double t){m_theta = t;}
		void set_landmark(bool t){m_landmark = t;}
		void set_filter(bool t){m_filter = t;}

		void   set_goal_set(const std::vector<aptk::Fluent_Vec> goal_set){
      m_goal_set = goal_set;
    }
    ...
    static std::vector<double> priors(const STRIPS_Problem& p) {return p.m_priors;}
    static float beta(const STRIPS_Problem& p) {return p.m_beta;}
    static std::vector<aptk::Fluent_Vec> goal_set(const STRIPS_Problem& p) {return p.m_goal_set;}
		...
		float get_df() const {return m_discount_factor;}
  ...
  public:
    ...
    Fluent_Vec m_true_init;
    aptk::STRIPS_Problem * m_p_prob;
    void set_p_prob(aptk::STRIPS_Problem * p){m_p_prob = p;}
    ...
    std::vector<aptk::Fluent_Vec> m_goal_set;
