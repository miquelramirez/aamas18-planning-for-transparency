...
#ifndef __DYNAMIC_NOVELTY_PARTITION__
#define __DYNAMIC_NOVELTY_PARTITION__
...
class Dynamic_Novelty_Partition : public Heuristic<State>{
public:


	Dynamic_Novelty_Partition( const Search_Model& prob, unsigned max_arity = 1, const unsigned max_MB = 2048 ) 
	...
	virtual ~Dynamic_Novelty_Partition() {
	}


	void init() {
	    typedef typename std::vector< std::unordered_map<unsigned,Search_Node*> >::iterator        Node_Map_Ptr_It;
	    
				
	    for(Node_Map_Ptr_It it_p = m_nodes_tuples_by_partition.begin();it_p != m_nodes_tuples_by_partition.end(); it_p++)
		it_p->clear();
	}
	...
	void set_arity( unsigned max_arity, unsigned partition_size = 0 ){
		...
		m_nodes_tuples_by_partition.resize( m_num_tuples );
...
protected:
	...
	bool cover_tuples( Search_Node* n, unsigned arity  )
	{
		...
		for( unsigned idx = 0; idx < n_combinations; idx++ ){
			 ...
			 * -> n better than old_n
			 */
			auto n_table = m_nodes_tuples_by_partition[ tuple_idx ].find(n->partition());
			bool cover_new_tuple = ( n_table == m_nodes_tuples_by_partition[ tuple_idx ].end() ) ? true : ( is_better( n_table->second, n  ) ? true : false);
			
			if( cover_new_tuple ){
				m_nodes_tuples_by_partition[ tuple_idx ][ n->partition() ] = (Search_Node*) n;
	...
	const STRIPS_Problem&	m_strips_model;
        std::vector< std::unordered_map<unsigned,Search_Node*> >     m_nodes_tuples_by_partition;
        unsigned                m_arity;
...
#endif // dynamic_novelty_partition.hxx
