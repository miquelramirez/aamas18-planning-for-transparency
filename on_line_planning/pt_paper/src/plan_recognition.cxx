...
namespace pr {
  ...
  std::vector<double> PlanRecognitionEngine::plan_recognition(std::vector<unsigned int> observations, aptk::Fluent_Vec& initial_state, float initial_beta, float& delta,
      std::vector<std::vector<int>>& best_negated_obs_rel_plan, std::vector<std::vector<unsigned int>>& best_negated_obs_plan, std::vector<float>& best_negated_obs_cost,
      std::vector<unsigned int> full_observations){

    if(observations.size() == 0){
      return m_goal_set_priors;
    }
    ...
    std::vector<int> delta_values = find_delta_values(observations, initial_state, best_negated_obs_rel_plan, best_negated_obs_plan, best_negated_obs_cost, full_observations);
    float delta_before = delta;

    float beta = initial_beta;
    if(m_variable_beta){
      beta = find_new_beta(delta_values, initial_beta, delta);
    }

    float delta_after = delta;
    if(m_verbose){
    ...
    //if(delta_before != delta_after){
      //for(auto i : m_goal_set_priors){
        //double invert_i = pow((1-i),(1-(1/(delta_after - delta_before))));

        //new_priors.push_back(i*invert_i);

      //}
      //m_goal_set_priors = new_priors;
    //}
  ...
  std::vector<int> PlanRecognitionEngine::find_delta_values(std::vector<unsigned int> observations, aptk::Fluent_Vec& initial_state,
    ...
    if(!m_approximate_pr){
      ...
      if(!m_s_problem->m_markov)
        clone_problem(* m_s_problem, transformed_s_problem_negative, m_s_problem->m_true_init);
      else
        clone_problem(* m_s_problem, transformed_s_problem_negative, initial_state);
    }
      ...
      */
      ...
      if(!m_approximate_pr){
        ...
        if(!m_negated_as_neutral){
          ...
          //std::cout << std::endl;
          //if (observations.size() > 0)
            //std::cout << "\t\t" << m_s_problem->actions()[observations[0]]->signature() << std::endl;

          cost_of_goal_wo_observations = approximation_wo_delete_relax_break_sequence(transformed_s_problem_negative,
              observations, m_verbose, best_negated_obs_rel_plan, best_negated_obs_plan, best_negated_obs_cost, full_observations, i);
 ...
 float PlanRecognitionEngine::find_new_beta(std::vector<int> delta_values, float initial_beta, float& delta_sum){

    auto goal = m_s_problem->goal();
    std::sort(goal.begin(), goal.end());
    auto it = std::find(m_candidate_goal_set.begin(), m_candidate_goal_set.end(), goal);
    int index;
    int min_ele;
    //std::cout << "Here1" << std::endl;
    //for(auto i : m_candidate_goal_set){
      //std::cout << print_fluent_vec(*m_s_problem, i) << std::endl;
      //std::cout  << std::endl;
    //}
    //print_fluent_vec(*m_s_problem, goal);


    if(m_s_problem->m_markov){
      if (it == m_candidate_goal_set.end())
      {
        ; //TODO
      } else
      {
        index = std::distance(m_candidate_goal_set.begin(), it);
      }

      if(m_verbose)
        std::cout << "Index = " << index << std::endl;

      min_ele = delta_values[index];
    }else{
      min_ele = *std::min_element( delta_values.begin(), delta_values.end() );
    }
    ...
    if(m_verbose){
      ...
      std::cout << "Min_Ele: " << min_ele << std::endl;
      std::cout << "Delta Sum: " << delta_sum << std::endl;
      std::cout << "Initial_beta: " << initial_beta << std::endl;
      std::cout << "Beta: " << initial_beta / pow(delta_sum + 1, n) << std::endl;
    }


    float beta = initial_beta / pow(delta_sum + 1, n);
  ...
  float PlanRecognitionEngine::approximation_wo_delete_relax_break_sequence(aptk::STRIPS_Problem& trans_prob_neg,
      ...
      std::vector<std::vector<int>>& best_negated_obs_rel_plan, std::vector<std::vector<unsigned int>>& best_negated_obs_plan, std::vector<float>& best_negated_obs_cost,
      std::vector<unsigned int> full_observations, unsigned int goal_index){

    //std::cout << "Init: " << print_fluent_vec(trans_prob_neg, trans_prob_neg.init()) << std::endl;
    //std::cout << "Observations: " << trans_prob_neg.actions()[observations[0]]->signature() << std::endl;
    ...
    //}
    //std::cout << "FOR GOAL " << goal_index << std::endl;
    ...
    for (unsigned int i = 0; i < observations.size(); i++ ) {
      ...
      trans_prob_neg.make_action_tables();
      ...
      if(m_verbose){
        std::cout << "\t\t\t\tInitial State: ";
        for(unsigned int i = 0 ; i < neg_search_prob.init()->fluent_vec().size(); i++){
          if ( i == neg_search_prob.init()->fluent_vec().size() - 1)
            std::cout  << neg_search_prob.task().fluents()[neg_search_prob.init()->fluent_vec()[i]]->signature() << std::endl;
          else
            std::cout  << neg_search_prob.task().fluents()[neg_search_prob.init()->fluent_vec()[i]]->signature() << ", ";
        }
      }
      heuristic_estimate(neg_search_prob, neg_cost, plan, observations);
      //do_pr_search(neg_search_prob, plan, neg_cost);
      auto i_nit = neg_search_prob.init()->fluent_vec();
      ...
      //std::cout << print_fluent_vec(trans_prob_neg, a->add_vec()) << std::endl;
      //std::cout << print_fluent_vec(trans_prob_neg, root.fluent_vec()) << std::endl;
    ...
    if(min < best_negated_obs_cost[goal_index]){
      best_negated_obs_cost[goal_index] = min;
      best_negated_obs_rel_plan[goal_index] = min_plan;
      full_observations.pop_back();
      best_negated_obs_plan[goal_index] = full_observations;
    }else{
      min = best_negated_obs_cost[goal_index];
      min_plan = best_negated_obs_rel_plan[goal_index];
      full_observations = best_negated_obs_plan[goal_index];
    }

    if(m_verbose){

      std::cout << "\t\t\t\tRelaxed Plan Found: ";
      for(unsigned j = 0; j < full_observations.size(); j++)
        std::cout << m_s_problem->actions()[full_observations[j]]->signature() << ",";
      std::cout << " | ";
    ...
    trans_prob_neg.fluents().pop_back();
