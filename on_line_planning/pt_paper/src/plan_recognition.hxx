...
namespace pr {
  ...
  public:
    ...
    std::vector<double> plan_recognition(std::vector<unsigned int> observations, aptk::Fluent_Vec& initial_state, float beta, float& delta_value,
    ...
    void set_goal_set_priors(std::vector<double> p){m_goal_set_priors = p;}
  ...
  private:
    ...
    float find_new_beta(std::vector<int> deltas, float initial_beta, float& delta_sum);
