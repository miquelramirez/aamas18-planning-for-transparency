...
#ifndef __AT_BFS_RECOG_HXX__
#define __AT_BFS_RECOG_HXX__
...
public:
	...
	//void  set_gn(float new_mg) {m_g = new_mg;}
  ...
  void set_og(std::vector<double> priors) {m_priors = priors;}
	...
	const Node<State>*		parent() const   	{ return m_parent; }
...
public:
	...
	bool	find_solution( float& cost, std::vector<Action_Idx>& plan, std::vector<double>& prob_vector) {
		m_t0 = time_used();
		Search_Node* end = do_search();
		if ( end == NULL ) return false;
		extract_plan( m_root, end, plan, cost, prob_vector );
	...
	bool 		is_closed( Search_Node* n ) 	{
      ...
      //eval(n);
      //float new_mg = n->gn() + n->hn();
      //n->m_g = new_mg;

      //std::cout << "n2h = " << n2->hn() << " nh = " << n->hn() << std::endl;
      //std::cout << "n2g = " << n2->gn() << " ng = " << n->gn() << std::endl;
      if ( n2->gn() <= n->gn() ) {
  ...
  Search_Node* 		get_node() {
	...
	virtual Search_Node*	 	do_search() {
		...
		while(head) {
      /*
       * TODO: Fix added 1 each actions
       */
      float new_mg = head->gn() + head->hn();
      //std::cout << "Count: " << counter << std::endl;
      //std::cout << "------------------------------\n";
      //std::cout << "H = " << head->hn() << std::endl;
      //std::cout << "G = " << head->gn() << std::endl;
      head->m_g = new_mg;
      //std::cout << "After G = " << head->gn() << std::endl;
      //std::cout << std::endl;
...
protected:
	...
	void	extract_plan( Search_Node* s, Search_Node* t, std::vector<Action_Idx>& plan, float& cost, std::vector<double>& prob_vector ) {
		Search_Node *tmp = t;
		cost = 0.0f;
		while( tmp != s) {
			cost += m_problem.cost( *(tmp->state()), tmp->action() );
			prob_vector.push_back(tmp->hn());
			plan.push_back(tmp->action());
			tmp = tmp->parent();
		}

		std::reverse(plan.begin(), plan.end());
		std::reverse(prob_vector.begin(),prob_vector.end());
