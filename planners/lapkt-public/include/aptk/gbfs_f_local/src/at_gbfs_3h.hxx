...
public:
	...
	: m_problem( search_problem ), m_exp_count(0), m_gen_count(0), m_pruned_B_count(0),
	  m_dead_end_count(0), m_open_repl_count(0),m_B( infty ), m_time_budget(infty), m_lgm(NULL), m_max_h2n(no_such_index), m_max_h3n(no_such_index), m_verbose( true ), m_ignore_novelty(false) {	
	...
	void			eval( Search_Node* candidate ) {
		...
		//candidate->fn() = (100 * candidate->h2n() + 10 * candidate->h3n()) + (1 * !candidate->is_helpful());
		// if( candidate->is_helpful() )
		//     candidate->fn() = (candidate->h2n() +  candidate->h3n()) * (1);		
		// else
		//     candidate->fn() = (candidate->h2n() +  candidate->h3n()) * (10);
		
		//candidate->fn() = (100 * candidate->h2n() ) + (10 * !candidate->is_helpful());
		candidate->fn() = (10000 * candidate->h2n() + candidate->h3n());// + (10 * !candidate->is_helpful());
		candidate->goals_unachieved() =  candidate->fn() ;
